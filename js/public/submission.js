...
function judge() {
    ...
    for (let i = 0; i < outlink_names.length; i++) {
    ...
    let abouttext = about.value.replace(/\n/g, '<br />');
    ...
    const formconfirm = new Popup();
    formconfirm.setyescollback(function () {
        result = loadXMLDoc(upload_url, formdata);
        if (result['status']) {
    ...
    formconfirm.setnocollback(function () {
        tips.innerHTML = '取消上传,请确认后再上传';
    });

    formconfirm.confirm('neri小窝的提交提示', '是否确认提交？', '确认', '取消');
}
...
$("#img_input").on("change", function (file_image) {
    ...
    reader.onload = function (arg) {
        let count_file = document.querySelectorAll(".preview").length;
